...
def dGdt_array(VBMGxt = [], *args):
    ...
    leftX[0] = centerX[0]
    rightX[-1] = centerX[-1]
    dG_dt_array[:] = +(inRateG + alva.event_OAS_boost + alva.event_OAS_boostV)*B[:]                      - consumeRateG*G[:]*crossI_neighborSum_X(V, cross_radius, gX)                     - outRateG*G[:]                      + mutatRateB*(leftX[:] - 2*centerX[:] + rightX[:])/(dx**2)
    return(dG_dt_array)
