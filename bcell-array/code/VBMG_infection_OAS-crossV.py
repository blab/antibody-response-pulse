...
plt.text(0, 3.0/9,r'$ \frac{\partial M_n(t)}{\partial t} =          + \xi_{m} B_{n}(t)          - \phi_{m} M_{n}(t) V_{n}(t)          - \mu_{m} M_{n}(t) $'
         , fontsize = 1.2*AlvaFontSize)
plt.text(0, 1.0/9,r'$ \frac{\partial G_n(t)}{\partial t} =          + \xi_{g} \sum_{j = 1}^{N} (1 - \frac{|j - n|}{r + |j - n|})B_{n}(t)          - \phi_{g} G_{n}(t) V_{n}(t)           - \mu_{g} G_{n}(t) $'         
         , fontsize = 1.2*AlvaFontSize)
...
mutatRateB = 0.00000/hour # Virus mutation rate

cross_radius = float(0.002) # radius of cross-immunity (the distance of half-of-value in the Monod equation)
...
actRateBg_OAS_press = -0.0005/hour # depress act-rate from memory B-cell for non-origin-virus
